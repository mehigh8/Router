...
// Structura unui nod din trie
typedef struct node_t {
	void* route; // Adresa spre o ruta din tabela de rutare
	struct node_t* zero; // Fiul stang, care reprezinta bitul 0
	struct node_t* one; // Fiul drept, care reprezinta bitul 1
} node;

/**
 * create_node - Functie care aloca memorie pentru un nod si ii seteaza ruta nodului.
 * @route - ruta ce va fi retinuta in nod.
 * return - nodul tocmai creat
 */
node* create_node(void* route);

/**
 * add_route - Functie care adauga o ruta in trie, parcurgand bit cu bit prefixul,
 * pana cand masca devine 0.
 * @root - nodul curent (radacina subarborelui curent)
 * @route - adresa rutei ce trebuie adaugata in trie
 * @mask - masca rutei
 * @prefix - prefixul rutei
 */
void add_route(node* root, void* route, uint32_t mask, uint32_t prefix);

/**
 * search_route - Functie care cauta ruta potrivita pentru o adresa ip, parcurgand trie-ul.
 * @root - nodul curent (radacina subarborelui curent)
 * @dest_ip - adresa ip pentru care se cauta ruta
 * @last - ultima ruta gasita in parcurgerea trie-ului
 * return - ruta gasita (sau NULL daca e cazul)
 */
