...
node* create_node(void* route)
{
	// Aloc memorie pentru nod.
	node* current = malloc(sizeof(node));
	DIE(current == NULL, "trie node memory");
	// Setez fiii nodului pe NULL.
	current->zero = NULL;
	current->one = NULL;
	// Setez ruta.
	current->route = route;
...
void add_route(node* root, void* route, uint32_t mask, uint32_t prefix)
{
	// Daca masca este 0, setez ruta nodului si ies din functie.
	if (mask == 0) {
		root->route = route;
		return;
	}
	// Preiau bitul curent (primul bit din prefix).
	int current_bit = prefix & (1 << 31);
	// Daca primul bit este 0, aloc memorie pentru nodul zero, si reapelez functia
	// shiftand la stanga cu 1 prefixul si masca.
	if (current_bit == 0) {
		if (root->zero == NULL) {
			root->zero = create_node(NULL);
		}
		add_route(root->zero, route, mask << 1, prefix << 1);
	// Daca primul bit este 1, aloc memorie pentru nodul one, si reapelez functia
	// shiftand la stanga cu 1 prefixul si masca.
	} else {
		if (root->one == NULL) {
			root->one = create_node(NULL);
		}
		add_route(root->one, route, mask << 1, prefix << 1);
	}
}

void* search_route(node* root, uint32_t dest_ip, void* last)
{
	// Daca am ajuns la un nod NULL inseamna ca am ajuns la o frunza, deci pot
	// intoarce ultima ruta gasita.
	if (root == NULL)
		return last;
	// Daca gasesc o ruta noua o retin.
	if (root->route != NULL)
		last = root->route;
	// Preiau bitul curent (primul bit din adresa ip).
	int current_bit = dest_ip & (1 << 31);
	// Daca primul bit este zero, reapelez functia pentru nodul zero si shiftez
	// adresa ip cu 1 la stanga.
	if (current_bit == 0) {
		return search_route(root->zero, dest_ip << 1, last);
	}
	// Analog pentru bitul 1, reapelez pentru nodul one.
	return search_route(root->one, dest_ip << 1, last);
}
